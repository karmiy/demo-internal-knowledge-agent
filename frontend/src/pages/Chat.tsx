  ...
  return (
    <div className="chat-page">
      ...
      <section className="conversation" aria-live="polite">
        ...
        {messages.map((message) => <article key={message.id} className={`message ${message.role}`}><span>{message.role === "user" ? "YOU" : "AGENT"}</span><div><p>{message.text}</p>{message.result && <><Citations items={message.result.citations} /><div className="activity">{message.result.activity.map((item) => <small key={item}>{item === "searched_documents" ? "已检索授权文档" : "已查询授权数据"}</small>)}</div></>}</div></article>)}
